...
public class FindDuplicates {
    ...
    such as MD5 or SHA. Message digests are secure one-way hash functions that take arbitrary-sized
    data and output a fixed-length hash value.  I needed this because I was running out of memory looking at
    file contents.
     */
    ...
    // Recursive function find takes a map list and a directory and uses an algorithm to determine what is a duplicate
    public static void find(Map<String, List<String>> lists, File directory, boolean leanAlgorithm) throws Exception  {
        // create a hash variable that is a string.
        String hash;
        // for each file in the directory....
        for (File child : directory.listFiles()) {
            // if the child is a directory itself, go into that directory / call this method
            if (child.isDirectory()) {
                find(lists, child, leanAlgorithm);
            } else {
                try {
                    // determine if the algorithm is meant to be quick and use more memory or use less
                    // memory and go slower (lean)
                    hash = leanAlgorithm ? makeHashLean(child) : makeHashQuick(child);
                    // set up the hash table if need be
                    // add the latest hash of a file to the list
